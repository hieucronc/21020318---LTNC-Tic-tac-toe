...
const SDL_Color PLAYER_X_COLOR = { .r = 209, .g = 26, .b = 26};
const SDL_Color PLAYER_O_COLOR = { .r = 32, .g = 75, .b = 168};
...
void render_O(SDL_Renderer *gRenderer, int x, int y, const SDL_Color *color)
{
	...
	radius = (CELL_HEIGHT/5);
	SDL_SetRenderDrawColor( gRenderer, 255, 197, 153, 0);
	for(int w = 0; w < radius *2; ++w)
	{
		for(int h = 0; h < radius * 2; ++h)
		{
